...
def KMeans(data, K, return_all_iterations=False):
    ...
    while True:
        ...
        for n in range(N):
            x = data[n, :]
            #########   EDIT HERE START   #########
            distances_to_m = np.sum((x - m)**2, axis=1)
            a[n] = np.argmin(distances_to_m) # needs to be the index k for which the distance between x and m_k is minimal
            #########   EDIT HERE END   #########
        ...
        #########   EDIT HERE START   #########
        for k in range(K):
            m[k, :] = np.mean(data[a == k], axis = 0) # change this to be the updated cluster means
        #########   EDIT HERE END   #########
...
if __name__ == "__main__":
    ...
    if option == 1:
        ...
        import pandas as pd        
        #########   EDIT HERE START   #########
        df = pd.read_csv('faithful.csv', index_col = 0) # construct a pandas dataframe from the csv-file "faithful.csv"
        print(df) # see what it looks like
        data = df.values # extract the data matrix from the dataframe df
        #########   EDIT HERE END   #########
    ...
    if M == 2:
        if m_history is not None:
            ...
            plt.tight_layout()
            plt.savefig('kmeans_overall.png')
            ...
            for it in range(num_its-1):
                ...
                plt.title("n = " + str(it) + ", after update")
                plt.tight_layout()
                plt.savefig(f'kmeans_visualization_{it}.png')
                plt.show()
