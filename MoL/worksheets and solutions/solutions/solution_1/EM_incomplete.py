...
def EM(data, K, n_iter = 25, return_all_iterations=False):
    ...
    for it in range(n_iter):
        ...
        for n in range(N):
            #########   EDIT HERE START   #########
            x = data[n, :]
            pdf = [p[k] / sqrt((2 * pi) ** M * np.linalg.det(Sigma[k])) * np.exp(-1/2 * np.dot(np.dot(x - m[k,:], np.linalg.inv(Sigma[k])), x - m[k,:])) for k in range(K)]
            gamma[n] = pdf / np.sum(pdf)
            #########   EDIT HERE END   #########
        ...
        m_new = np.dot(gamma.T, data) / Nk # compute m_new (new cluster means)
        #########   EDIT HERE END   #########
...
if __name__ == "__main__":
    ...
    def drawfig(it):
        ...
        plt.savefig(f'EM_visualization_{it}.png')
        plt.show()
    
    for i in [0, m_history.shape[0]-2]:
        drawfig(i)
