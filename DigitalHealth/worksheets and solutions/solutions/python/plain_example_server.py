# -*- coding: utf-8 -*-
"""
Created on Tue Dec 15 08:28:06 2020
...
def test_api_request():
  ...
  # body = {
  #     "dataStreamName": "CoffeeTestSource",
  #     "type": "raw",
  #     "application": {
  ...
  #     }
  # }
  # files = fit.users().dataSources().create(userId='me', body=body).execute()
  # body = json.load(open('coffee_data_example_DT.json'))
  data_source_id = 'raw:com.google.nutrition:635848177559:CoffeeTestSource'
  # files = fit.users().dataSources().datasets()\
  ...
  dt_end = datetime(2015, 6, 1, 23, 59)
  body = {
      "aggregateBy": [
          {
  ...
  files = fit.users().dataset().aggregate(userId='me', body=body).execute()
  with open('file_aggregated_coffe.json', 'w') as f:
      json.dump(files, f)

  fit.users().dataSources().datasets()\
      .get(userId='me', body=data_sorce_id, data_set_id).execute()
